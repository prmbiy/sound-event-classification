...
def run(feature_type, num_bins, data_type):
    ...
    for file_name in tqdm(file_list):
        data = np.load('./data/{}/{}/{}.npy'.format(feature_type, data_type, file_name))
        x = data.mean(axis=1)
        n += 1
        delta = x - mean
        mean = mean + delta/n
...
if __name__=="__main__":
    ...
    run(args.feature_type, args.num_bins, args.data_type)
